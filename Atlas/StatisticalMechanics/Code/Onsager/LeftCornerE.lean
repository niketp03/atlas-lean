...
namespace StatMech.Onsager.LeftCornerE
...
theorem cornerWeight_leftTurn_E (d : Fin n → Fin 4) (hclosed : ∑ i, stepOf (d i) = 0)
    (hsimple : Function.Injective (pos d)) (hn : 3 ≤ n) (i : Fin n)
    (hei : d i = 3) (he : d (i + 1) = 0) :
  ...
  have hedgei_str : i ∈ straddleSet d (vx - 1) vy := by
  ...
  have hNE : rayParity d (vx - 1) vy = rayParity d vx vy + 1 := by
    have h := rayParity_hflip d hclosed (vx - 1) vy
      (straddleSet_card_odd_of_mem d hclosed hsimple hn (vx - 1) vy i hedgei_str)
    rwa [show vx - 1 + 1 = vx from by ring] at h
  
  have hedgei1_mid : i + 1 ∈ midSet d vx vy := by
    simp only [midSet, mem_filter, mem_univ, true_and]
    refine ⟨Or.inl he, ?_, ?_⟩
    · rw [hposi2, show (pos d (i + 1)).1 = vx from hvx.symm]; omega
    · rw [show (pos d (i + 1)).2 = vy from hvy.symm]
  have hSE : rayParity d vx (vy - 1) = rayParity d vx vy + 1 := by
    have h := rayParity_vflip d vx (vy - 1)
      (by rw [show vy - 1 + 1 = vy from by ring]
          exact midSet_card_odd_of_mem d hclosed hsimple hn vx vy (i + 1) hedgei1_mid)
  ...
  have hSW : rayParity d (vx - 1) (vy - 1) = rayParity d (vx - 1) vy := by
    have hstr : ∀ k : Fin n, isHorizEdge (d k) →
      ...
      · 
        ...
        rw [hki] at hmk
        rw [hx2, show (pos d (i + 1)).1 = vx from hvx.symm] at hmk
        omega
    have h := rayParity_vstep d (vx - 1) (vy - 1) hstr
    rwa [show vy - 1 + 1 = vy from by ring] at h
  
  have hpe : pos d (i + 1) = (vx, vy) := Prod.ext hvx.symm hvy.symm
  have hNW : leftCell (d (i + 1)) (pos d (i + 1)) = (vx, vy) := by
    rw [he]; simp only [leftCell]; exact hpe
  ...
  show cornerWeight (cornerCount (interiorCells d hclosed) (pos d (i + 1)))
      = if rayParity d vx vy = 1 then (1 : ℤ) else -1
  ...
  · have e1 : rayParity d (vx - 1) vy = 1 := by rw [hNE, h0]; decide
    ...
    have e3 : rayParity d (vx - 1) (vy - 1) = 1 := by rw [hSW, hNE, h0]; decide
  ...
  · have e1 : rayParity d (vx - 1) vy = 0 := by rw [hNE, h1]; decide
    ...
    have e3 : rayParity d (vx - 1) (vy - 1) = 0 := by rw [hSW, hNE, h1]; decide
...
end StatMech.Onsager.LeftCornerE
