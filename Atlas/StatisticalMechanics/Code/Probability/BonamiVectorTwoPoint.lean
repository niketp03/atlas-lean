...
theorem bvtp_qnorm2_eq_sqrt (u : (ι → Bool) → ℝ) :
    bnt_qnorm 2 u = Real.sqrt (bnt_qnormPow 2 u) := by
  unfold bnt_qnorm; rw [Real.sqrt_eq_rpow]



theorem bvtp_qnorm2_sq (u : (ι → Bool) → ℝ) :
    (bnt_qnorm 2 u) ^ 2 = bnt_qnormPow 2 u := by
  rw [bvtp_qnorm2_eq_sqrt, Real.sq_sqrt (bnt_qnormPow_nonneg 2 u)]
...
theorem bvtp_qnormPow2_expand (a b : ℝ) (u v : (ι → Bool) → ℝ) :
    bnt_qnormPow 2 (fun ω => a * u ω + b * v ω)
      = a ^ 2 * bnt_uexp (fun ω => (u ω) ^ 2)
        + 2 * a * b * bnt_uexp (fun ω => u ω * v ω)
        + b ^ 2 * bnt_uexp (fun ω => (v ω) ^ 2) := by
  rw [bvtp_qnormPow2_eq_uexp_sq]
  have heq : (fun ω => (a * u ω + b * v ω) ^ 2)
      = (fun ω => a ^ 2 * (u ω) ^ 2 + (2 * a * b) * (u ω * v ω) + b ^ 2 * (v ω) ^ 2) := by
    funext ω; ring
  rw [heq, bvtp_uexp_add, bvtp_uexp_add, bvtp_uexp_smul, bvtp_uexp_smul, bvtp_uexp_smul]




theorem bvtp_uexp_inner_le (u v : (ι → Bool) → ℝ) :
    bnt_uexp (fun ω => u ω * v ω) ≤ bnt_qnorm 2 u * bnt_qnorm 2 v := by
  rw [bvtp_qnorm2_eq_sqrt, bvtp_qnorm2_eq_sqrt, bvtp_qnormPow2_eq_uexp_sq,
    bvtp_qnormPow2_eq_uexp_sq]
  unfold bnt_uexp
...
theorem bvtp_vectorTwoPoint_aux {ρ : ℝ} (h0 : 0 ≤ ρ) (h1 : ρ ≤ 1)
  ...
  have hp2 : p ^ 2 = Eu := by rw [hp, bvtp_qnorm2_sq, bvtp_qnormPow2_eq_uexp_sq]
  have hs2 : s ^ 2 = Ev := by rw [hs, bvtp_qnorm2_sq, bvtp_qnormPow2_eq_uexp_sq]
  
  have hexp1 := bvtp_qnormPow2_expand ((1 + ρ) / 2) ((1 - ρ) / 2) u v
  have hexp2 := bvtp_qnormPow2_expand ((1 - ρ) / 2) ((1 + ρ) / 2) u v
  ...
  have hCS : Euv ≤ p * s := bvtp_uexp_inner_le u v
...
theorem bvtp_vectorTwoPoint {ρ : ℝ} (h0 : 0 ≤ ρ) (h1 : ρ ≤ 1) :
    bnt_VectorTwoPoint ρ :=
  fun n u v => bvtp_vectorTwoPoint_aux h0 h1 n u v
...
theorem bvtp_tensorStep {ρ : ℝ} (h0 : 0 ≤ ρ) (h1 : ρ ≤ 1) :
    bnt_TensorStep ρ :=
  bnt_TensorStep_of_vectorTwoPoint (bvtp_vectorTwoPoint h0 h1)
...
theorem bvtp_hypercontractivity {ρ : ℝ} (h0 : 0 ≤ ρ) (h1 : ρ ≤ 1)
    (n : ℕ) (f : (Fin n → Bool) → ℝ) :
    bnt_qnorm 2 (bnt_noiseOp ρ f) ≤ bnt_qnorm (1 + ρ ^ 2) f :=
  bnt_hypercontractivity_of_twoPoint (bvtp_tensorStep h0 h1) n f
