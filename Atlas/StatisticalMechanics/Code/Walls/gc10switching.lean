...
theorem gc10_switching_difference_to_delta (ends : ι → Sym2 V) (F : Finset ι → ℝ) (A : Finset V)
...
theorem gc10_switching_difference_to_disconn (ends : ι → Sym2 V) (F : Finset ι → ℝ) (A : Finset V)
    (hnd : ∀ i, ¬ (ends i).IsDiag) {o g : V} (hog : o ≠ g) :
...
theorem gc10_switching_difference_to_gate (ends : ι → Sym2 V)
  ...
  gc9_switching_difference_to_gate ends hnd A hog F
...
theorem gc10_eqswi_vanishing_branch (ends : ι → Sym2 V) (M : Finset ι)
...
theorem gc10_eqswi_count_collapse (ends : ι → Sym2 V) (M : Finset ι)
...
theorem gc10_switching_eqswi (ends : ι → Sym2 V) (F : Finset ι → ℝ) (A : Finset V)
  ...
  refine ⟨gc10_switching_difference_to_delta ends F A hnd hog, ?_⟩
  exact gc10_eqswi_count_collapse ends Finset.univ (fun i _ => hnd i) B
...
theorem gc10_delta_eq_switching_gap (ends : ι → Sym2 V) (F : Finset ι → ℝ) (A : Finset V)
    (hnd : ∀ i, ¬ (ends i).IsDiag) {o g : V} (hog : o ≠ g) :
    drd_pairSum ends A F (fun m => gc8_disconn ends o g m)
      = (∑ m ∈ (Finset.univ.powerset.filter (fun m => RandomCurrent.sources ends m = A)),
          (∑ _K ∈ m.powerset.filter (fun K => RandomCurrent.sources ends K = A), F m))
        - (∑ m ∈ (Finset.univ.powerset.filter (fun m => RandomCurrent.sources ends m = A)),
            (∑ _K ∈ m.powerset.filter (fun K => RandomCurrent.sources ends K = A ∆ {o, g}), F m)) :=
  gc10_switching_difference_to_delta ends F A hnd hog
...
theorem gc10_switching_difference_to_delta_nonvacuous :
        ...
        - (∑ m ∈ (Finset.univ.powerset.filter
            ...
            (∑ _K ∈ m.powerset.filter
              (fun K => RandomCurrent.sources witEnds K
                ...
                (fun _ => (1 : ℝ)) m)) :=
  gc10_switching_difference_to_delta witEnds (fun _ => 1) ({0, 1, 2, 3} : Finset (Fin 4))
    (by decide) (by decide)
