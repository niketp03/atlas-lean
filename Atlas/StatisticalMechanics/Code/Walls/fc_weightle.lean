...
theorem fc_edgeProduct_nonneg {p : ℝ} (hp : 0 ≤ p) (hp1 : p ≤ 1)
...
theorem fc_edgeProduct_le_one {p : ℝ} (hp : 0 ≤ p) (hp1 : p ≤ 1)
...
theorem fc_qpow_numClusters_le {q : ℝ} (hq : 1 ≤ q) (ω : ConfigSpace (Sym2 V)) :
...
theorem fc_fkWeight_le_qpow_card {p q : ℝ} (hp : 0 ≤ p) (hp1 : p ≤ 1) (hq : 1 ≤ q)
    (ω : ConfigSpace (Sym2 V)) :
    fkWeight G p q ω ≤ q ^ Fintype.card V := by
  unfold fkWeight
  have hep_le : edgeProduct G p ω ≤ 1 := fc_edgeProduct_le_one G hp hp1 ω
  have hpow_le : q ^ numClusters G ω ≤ q ^ Fintype.card V :=
    fc_qpow_numClusters_le G hq ω
...
theorem fc_fkWeight_le_qpow_card' {p q : ℝ} (hp : 0 ≤ p) (hp1 : p ≤ 1) (hq : 1 ≤ q)
  ...
  rw [Finset.card_univ]
  exact fc_fkWeight_le_qpow_card G hp hp1 hq ω
