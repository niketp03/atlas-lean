...
def fc_T (m n : ℕ) : ℕ := (2 * n + 1) / (2 * m + 1)


theorem fc_T_mul_le (m n : ℕ) : fc_T m n * (2 * m + 1) ≤ 2 * n + 1 :=
  Nat.div_mul_le_self _ _


theorem fc_lt_T_succ_mul (m n : ℕ) : 2 * n + 1 < (fc_T m n + 1) * (2 * m + 1) := by
  have hdm := Nat.div_add_mod (2 * n + 1) (2 * m + 1)
  have hmod := Nat.mod_lt (2 * n + 1) (show 0 < 2 * m + 1 by omega)
  rw [fc_T]; nlinarith [hdm, hmod]




theorem fc_denom_tendsto_atTop :
...
theorem fc_packing_ratio_tendsto (m : ℕ) :
    Tendsto (fun n => ((fc_T m n : ℝ) * (2 * m + 1)) / (2 * n + 1)) atTop (𝓝 1) := by
  have hupper : ∀ n, ((fc_T m n : ℝ) * (2 * m + 1)) / (2 * n + 1) ≤ 1 := by
    intro n
    have hden : (0 : ℝ) < 2 * n + 1 := by positivity
    rw [div_le_one hden]
    have : (fc_T m n * (2 * m + 1) : ℕ) ≤ 2 * n + 1 := fc_T_mul_le m n
    exact_mod_cast this
  have h0 : Tendsto (fun n : ℕ => (2 * (m : ℝ) + 1) / (2 * n + 1)) atTop (𝓝 0) :=
    (tendsto_const_nhds (x := (2 * (m : ℝ) + 1))).div_atTop fc_denom_tendsto_atTop
  ...
  have hlower : ∀ n : ℕ, 1 - (2 * (m : ℝ) + 1) / (2 * n + 1)
      ≤ ((fc_T m n : ℝ) * (2 * m + 1)) / (2 * n + 1) := by
    intro n
    have hden : (0 : ℝ) < 2 * n + 1 := by positivity
    have hT1 : 2 * n + 1 < (fc_T m n + 1) * (2 * m + 1) := fc_lt_T_succ_mul m n
    have hT1R : (2 * (n : ℝ) + 1) < ((fc_T m n : ℝ) + 1) * (2 * m + 1) := by
      have : ((2 * n + 1 : ℕ) : ℝ) < (((fc_T m n + 1) * (2 * m + 1) : ℕ) : ℝ) := by
        exact_mod_cast hT1
...
theorem fc_oddEven_ratio_tendsto :
...
theorem fc_bulkFraction_arith_tendsto (hd : 1 ≤ d) (m : ℕ) (hm : 1 ≤ m) :
    Tendsto (fun n : ℕ => ((fc_T m n) ^ d : ℝ)
        * ((d : ℝ) * (2 * m) * (2 * (m : ℝ) + 1) ^ (d - 1))
        / ((d : ℝ) * (2 * n) * (2 * (n : ℝ) + 1) ^ (d - 1))) atTop
      (𝓝 (2 * (m : ℝ) / (2 * m + 1))) := by
  
  have hprod : Tendsto (fun n => (((fc_T m n : ℝ) * (2 * m + 1)) / (2 * n + 1)) ^ d
        * (2 * (m : ℝ) / (2 * m + 1)) * ((2 * (n : ℝ) + 1) / (2 * n))) atTop
      (𝓝 ((1 : ℝ) ^ d * (2 * (m : ℝ) / (2 * m + 1)) * 1)) :=
    (((fc_packing_ratio_tendsto m).pow d).mul tendsto_const_nhds).mul fc_oddEven_ratio_tendsto
...
theorem fc_bulkFraction_tendsto (hd : 1 ≤ d) (m : ℕ) (hm : 1 ≤ m) :
    Tendsto (fun n => ((fc_T m n) ^ d : ℝ) * ((boxGraph d m).edgeFinset.card : ℝ)
        / ((boxGraph d n).edgeFinset.card : ℝ)) atTop
      (𝓝 (2 * (m : ℝ) / (2 * m + 1))) := by
  have harith := fc_bulkFraction_arith_tendsto (d := d) hd m hm
