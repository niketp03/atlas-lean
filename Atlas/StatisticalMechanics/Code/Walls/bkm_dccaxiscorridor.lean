...
theorem bkm_axisCorridor_step_open (ω : ConfigSpace (Sym2 (Site d))) (j : Fin d) (L : ℕ)
    (s : ℕ) (h1 : 1 ≤ s) (hsL : s < L) :
    IsOpenEdge d (removeSite 0 (forceOpenFinset (hrHD_corridorEdges j L) ω))
      (hrHD_rayPt j (s : ℤ)) (hrHD_rayPt j ((s : ℤ) + 1)) :=
  bkm_dcc_step_open ω (hrHD_corridorEdges j L)
...
theorem bkm_axisCorridor_connected_to (ω : ConfigSpace (Sym2 (Site d))) (j : Fin d)
    (L : ℕ) (m : ℕ) (hm1 : 1 ≤ m) (hmL : m ≤ L) :
  ...
  | succ p ih =>
    ...
    · 
      ...
      have hstep : IsOpenEdge d ω' (hrHD_rayPt j (p : ℤ)) (hrHD_rayPt j ((p : ℤ) + 1)) :=
        bkm_axisCorridor_step_open ω j L p hp1 hppL
      have hrec : Connected d ω' (hrHD_rayPt j 1) (hrHD_rayPt j (p : ℤ)) :=
        ih hp1 (by omega)
...
theorem bkm_axisCorridor_survives_removeSite (ω : ConfigSpace (Sym2 (Site d))) (j : Fin d)
    (L : ℕ) (hL : 1 ≤ L) :
    Connected d (removeSite 0 (forceOpenFinset (hrHD_corridorEdges j L) ω))
      (hrHD_rayPt j 1) (hrHD_rayPt j (L : ℤ)) :=
  bkm_axisCorridor_connected_to ω j L L hL le_rfl
...
theorem bkm_farEnd_ne_origin (j : Fin d) (L : ℕ) (hL : 1 ≤ L) :
    (hrHD_rayPt j (L : ℤ) : Site d) ≠ 0 := by
  rw [Ne, hrHD_rayPt_eq_zero_iff]
  intro h
  have : L = 0 := by exact_mod_cast h
  omega




theorem bkm_farEnd_ne_neighbour (j : Fin d) (L : ℕ) (hL : 2 ≤ L) :
    (hrHD_rayPt j (L : ℤ) : Site d) ≠ hrHD_rayPt j 1 := by
  intro h
